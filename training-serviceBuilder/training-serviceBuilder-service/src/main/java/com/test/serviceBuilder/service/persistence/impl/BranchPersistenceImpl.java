...
import com.test.serviceBuilder.model.BranchTable;
import com.test.serviceBuilder.model.impl.BranchImpl;
import com.test.serviceBuilder.model.impl.BranchModelImpl;
import com.test.serviceBuilder.service.persistence.BranchPersistence;
import com.test.serviceBuilder.service.persistence.BranchUtil;
...
public class BranchPersistenceImpl
	...
	public Branch[] findByUuid_PrevAndNext(
		...
		Branch branch = findByPrimaryKey(Id);
	...
	private FinderPath _finderPathFetchByNameAndId;
	private FinderPath _finderPathCountByNameAndId;

	/**
	 * Returns the branch where Name = &#63; or throws a <code>NoSuchBranchException</code> if it could not be found.
	 *
	 * @param Name the name
	 * @return the matching branch
	 * @throws NoSuchBranchException if a matching branch could not be found
	 */
	@Override
	public Branch findByNameAndId(String Name) throws NoSuchBranchException {
		Branch branch = fetchByNameAndId(Name);
	...
	/**
	 * Returns the branch where Name = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param Name the name
	 * @return the matching branch, or <code>null</code> if a matching branch could not be found
	 */
	@Override
	public Branch fetchByNameAndId(String Name) {
		return fetchByNameAndId(Name, true);
	}

	/**
	 * Returns the branch where Name = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param Name the name
	 * @param useFinderCache whether to use the finder cache
	 * @return the matching branch, or <code>null</code> if a matching branch could not be found
	 */
	@Override
	public Branch fetchByNameAndId(String Name, boolean useFinderCache) {
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByNameAndId, finderArgs, this);
		}

		if (result instanceof Branch) {
			Branch branch = (Branch)result;

			if (!Objects.equals(Name, branch.getName())) {
				result = null;
			}
		}
		...
		if (result == null) {
			...
			if (Name.isEmpty()) {
				sb.append(_FINDER_COLUMN_NAMEANDID_NAME_3);
			}
			else {
				bindName = true;

				sb.append(_FINDER_COLUMN_NAMEANDID_NAME_2);
			}
			...
			try {
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByNameAndId, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"BranchPersistenceImpl.fetchByNameAndId(String, boolean) with parameters (" +
	...
	/**
	 * Removes the branch where Name = &#63; from the database.
	 *
	 * @param Name the name
	 * @return the branch that was removed
	 */
	@Override
	public Branch removeByNameAndId(String Name) throws NoSuchBranchException {
		Branch branch = findByNameAndId(Name);
	 ...
	 */
	@Override
	public int countByNameAndId(String Name) {
		Name = Objects.toString(Name, "");

		FinderPath finderPath = _finderPathCountByNameAndId;
	...
	private static final String _FINDER_COLUMN_NAMEANDID_NAME_2 =
		"branch.Name = ?";

	private static final String _FINDER_COLUMN_NAMEANDID_NAME_3 =
	...
	public void cacheResult(Branch branch) {
		...
		finderCache.putResult(
			_finderPathFetchByNameAndId, new Object[] {branch.getName()},
			branch);
	}
	...
	protected void cacheUniqueFindersCache(BranchModelImpl branchModelImpl) {
		...
		args = new Object[] {branchModelImpl.getName()};

		finderCache.putResult(
			_finderPathCountByNameAndId, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByNameAndId, args, branchModelImpl);
	}
	...
	public void activate() {
		...
		_finderPathFetchByNameAndId = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByNameAndId",
			new String[] {String.class.getName()}, new String[] {"Name"}, true);

		_finderPathCountByNameAndId = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByNameAndId",
